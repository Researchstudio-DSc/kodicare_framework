...
def update_res_change_map(res_change_map, evolving_dtc_parser, interval_start1, interval_end1, interval_start2,
    ...
    for system in SYSTEMS:
        for metric in METRICS:
            results = evolving_dtc_parser.get_avg_run_evaluation(system, 'dtc_evolving_eval', metric)
            res_interval1 = results[interval_start1:interval_end1]
            res_interval2 = results[interval_start2:interval_end2]
            res_change_map[system + '-' + metric].append(
                evolving_dtc_parser.is_significant_change(res_interval1, res_interval2))
...
class TFIDFTrainingDataBuilder(training_data_builder_interface.TrainingDataBuilderInterface):
    ...
    def build_rd_df(self):
        ...
        return pd.DataFrame.from_dict(res_change_map)
