...
def write_to_qrels(run_name, qrel_id_field, run_result_path, qrels_out_path):
    ...
    for query in run_data:
        ...
        for doc in query['relevant_docs']:
            qrel_id = doc[qrel_id_field]
            print(f"{query_id} Q0 {qrel_id} {rank} {doc['score']:.4f} {run_name}")
            qrels_list.append(f"{query_id} Q0 {qrel_id} {rank} {doc['score']:.4f} {run_name}")
            rank += 1
...
def main(cfg):
    ...
    qrel_id_field = cfg.retrieval.qrel_id_field
    qrels_out_path = io_util.join(cfg.config.root_dir, io_util.join(cfg.config.output_dir,
                                                                    cfg.retrieval.run_name + '.txt'))

    write_to_qrels(run_name, qrel_id_field, run_result_path, qrels_out_path)
